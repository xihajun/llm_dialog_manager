...
# 这里假设你已经有这个类，从你的示例代码里复制即可
from llm_dialog_manager import Agent

def time_to_seconds(time_str: str) -> int:
    """
    把 [HH:MM:SS] 或 [MM:SS] 转成秒数
    """
...
def parse_timestamped_lines(full_transcript: str) -> List[Tuple[int, int, str]]:
    """
    将每一行解析成 (start_second, end_second, line_text)，方便后续处理。

    full_transcript示例:
    [00:00 -> 00:04]  hi everyone so in this video I'd like ...
    [00:02 -> 00:06]  to cover the process of tokenization ...
    ...
    返回: List[(start_sec, end_sec, "原始行文本")]
    """
    lines_data = []
    ...
    for line in lines:
        ...
        if match:
            time_part = match.group(1)  # e.g. "00:00 -> 00:04"
            if '->' in time_part:
                start_str, end_str = time_part.split('->')
                start_str, end_str = start_str.strip(), end_str.strip()
                start_sec = time_to_seconds(start_str)
                end_sec = time_to_seconds(end_str)
                lines_data.append((start_sec, end_sec, line))
            else:
                # 如果没能解析正确的 -> 结构，就跳过或自行处理
                pass
        else:
            # 该行不含 [xx:xx -> yy:yy] 格式，决定是否要收纳？
            # 也可存个(-1, -1, line) or simply skip
            pass
    return sorted(lines_data, key=lambda x: x[0])  # 按开始时间排序

def split_transcript_with_overlap(full_transcript: str,
                                  chunk_duration: int = 600,
                                  overlap: int = 120) -> List[Tuple[str, str, str]]:
    """
    基于“滑动窗口 + 时间重叠”逻辑，切分转录文本，返回 (start_time_str, end_time_str, chunk_text)。

    chunk_duration: 单个区块的持续时间(秒)，比如600=10分钟。
    overlap: 相邻区块之间的重叠时长(秒)，例如120=2分钟。
    """
    # 先将全文解析为行级(time_start, time_end, line)
    lines_data = parse_timestamped_lines(full_transcript)
    if not lines_data:
        return []

    # 全局时间范围
    global_start = lines_data[0][0]
    global_end = max(line[1] for line in lines_data)

    chunks = []
    current_start_sec = global_start

    # 当 current_start_sec 超过最后全局时间，就不再产生新的分块
    while current_start_sec <= global_end:
        current_end_sec = current_start_sec + chunk_duration

        # 收集落入 [current_start_sec, current_end_sec] 区间内的行
        chunk_lines = []
        for (st_sec, ed_sec, text) in lines_data:
            # 如果行的开始时间在区间内或相交，就纳入当前块
            # 这里简单判定 st_sec < current_end_sec && ed_sec >= current_start_sec
            if not (ed_sec < current_start_sec or st_sec > current_end_sec):
                chunk_lines.append((st_sec, ed_sec, text))

        if not chunk_lines:
            # 没有行落入本区间，也要继续往下走
            # 直接把这个区块跳过
            current_start_sec += (chunk_duration - overlap)
            continue

        # 找到chunk内的最早起始和最晚结束时间，用来生成字符串
        chunk_min_sec = min(x[0] for x in chunk_lines)
        chunk_max_sec = max(x[1] for x in chunk_lines)

        # 将这些行合并成文本
        # 这里可以直接拼接，也可以只拼文本部分
        chunk_text = "\n".join(x[2] for x in chunk_lines)

        # 转成 [HH:MM:SS] 形式
        chunk_start_str = seconds_to_hhmmss(chunk_min_sec)
        chunk_end_str = seconds_to_hhmmss(chunk_max_sec)

        # 存入结果
        chunks.append((chunk_start_str, chunk_end_str, chunk_text))

        # 滑动窗口前进 chunk_duration - overlap
        # 这样下一段就会跟本段有 overlap 秒的重叠时间
        next_start_sec = current_start_sec + (chunk_duration - overlap)
        current_start_sec = next_start_sec

        # 如果 current_start_sec 已经超过 global_end，可能结束循环
        if current_start_sec > global_end:
            break
...
def process_chunk(chunk_data: Tuple[str, str, str], model_name: str = "DeepSeek-R1") -> Tuple[str, str, str]:
    """
    并行处理单个分块: 调用模型做摘要
    返回 (start_time, end_time, summary)
    """
...
5. 保证覆盖整个时间线，注意前一个章节的结束时间和下一个章节的开始时间衔接自然，不要遗漏时间信息
6. 对于被重复包含在前后两个chunk的内容，请避免在大纲中重复呈现
7. 章节内部的段落请保持在对应的时间范围内
...
## Section 1: (HH:MM:SS - HH:MM:SS)
- Key points or subtopics

## Section 2: (HH:MM:SS - HH:MM:SS)
- Key points or subtopics
...
"""
    ...
    # 1) 切分时用10分钟chunk+2分钟overlap做示例
    chunks = split_transcript_with_overlap(
        sample_transcript, 
        chunk_duration=600,  # 10分钟=600秒
        overlap=120          # 2分钟overlap
    )

    # 2) 并行处理所有分块并合并摘要
    final_outline = summarize_all_chunks_parallel(chunks, num_processes=32, model_name="DeepSeek-R1")
